...
{
    /*  0 */    &CDrillDlg::FillRead,           /* 读取数据 */
    /*  1 */    &CDrillDlg::FillMulti,          /* 校准倍数 */
    /*  2 */    &CDrillDlg::FillQipi,           /* 去皮 */
    /*  3 */    &CDrillDlg::FillCollect,        /* 采集数据 */
    /*  4 */    &CDrillDlg::FillShow,           /* 显示扭矩 */
    /*  5 */    &CDrillDlg::FillSpeed,          /* 减速扭矩 */
    /*  6 */    &CDrillDlg::FillControl,        /* 控制扭矩 */
    /*  7 */    &CDrillDlg::FillBSpeed,         /* 开始减速 */
    /*  8 */    &CDrillDlg::FillESpeed,         /* 停止减速 */
    /*  9 */    &CDrillDlg::FillBUnload,        /* 开始卸荷 */
    /* 10 */    &CDrillDlg::FillEUnload,        /* 停止卸荷 */
    /* 11 */    &CDrillDlg::FillUpper,          /* 上限扭矩 */
    /* 12 */    &CDrillDlg::FillLower,          /* 下限扭矩 */
    /* 13 */    &CDrillDlg::FillCut,            /* 打折系数 */
    /* 14 */    &CDrillDlg::FillCollectOK,      /* 收到有效数据再次采集数据：最后一次确认后不需要再次收到数据 */
    /* 15 */    &CDrillDlg::FillCollectNOK,     /* 收到无效数据再次采集数据 */
    /* 16 */    &CDrillDlg::FillTime,           /* 设置单片机时间 */
    /* 17 */    &CDrillDlg::FillValvePress,     /* 设置减压阀参数 */
    /* 18 */    &CDrillDlg::FillValveFlow,      /* 设置流量阀参数 */
    /* 19 */    &CDrillDlg::FillReadValve,      /* 读取阀门状态参数 */
    /* 20 */    &CDrillDlg::FillReadMulti,      /* 读取多组数据 */
    /* 21 */    &CDrillDlg::FillWriteCalib,     /* 写 分段校准 */
    /* 22 */    &CDrillDlg::FillCtrlCalib,      /* 控制分段校准 */
    /* 23 */    &CDrillDlg::FillReadCalib,      /* 读 分段校准 */
    /* 19       &CDrillDlg::FillValveZoom         设置阀放大倍数参数 */
};

/* 收集数据函数数组 */
BOOL(CDrillDlg::* g_CollectProc[4])(COLLECTDATA* ptCollData) =
{
    /*  0 */    &CDrillDlg::CollectPort,             /* 收集串口数据 */
    /*  1 */    &CDrillDlg::CollectRandData,         /* 随机测试数据 */
    /*  2 */    &CDrillDlg::CollectTorqData,         /* 现场自动记录扭矩数据 */
    /*  3 */    &CDrillDlg::CollectMultiTorq         /* 现场自动记录多组扭矩数据 */
};
...
BOOL CAboutDlg::OnInitDialog()
{
    ...
    else
    {
        ...
        theDlg.GetVolMacFromRegStr(m_strReg, adwRegVol, adwRegMac, iYear, iMonth, iDay);
    }

    /* 获取机器上的卷和MAC信息 */
    theDlg.GetVolMacInfo(adwPCVol, adwPCMac, iYear, iMonth, iDay);
...
void CAboutDlg::SaveRegFile()
{
    ...
    newfile.Write(&theDlg.m_dwTotalTorqNum, sizeof(DWORD));
    newfile.Close();
}
...
void CDrillDlg::DoDataExchange(CDataExchange* pDX)
{
    ...
    DDX_Text(pDX, IDC_EDITTOTAL, m_dwTotalTorqNum);
    DDX_Check(pDX, IDC_SETTOOLBUCK, m_bToolBuck);
    DDX_Text(pDX, IDC_QUALITY, m_strQuality);
    DDX_Radio(pDX, IDC_RADIOMAKEUP, g_tGlbCfg.iBreakOut);
...
BOOL CDrillDlg::OnInitDialog()
{
    ...
    if (0 == m_dwTotalTorqNum)
        m_dwTotalTorqNum = g_tReadData.nTotal;
...
void CDrillDlg::GetMakeupCurNum()
{
    ...
    if (file.GetLength() != 0)
    {
        ...
        file.Read(&nCurBreakout, sizeof(UINT));
    }
    file.Close();
...
void CDrillDlg::FinishSetStatus()
{
    ...
    /* 质量判定 */
    dwQuality = theApp.JudgeQuality(&m_tSaveData, g_tGlbCfg.iBreakOut);
...
void CDrillDlg::SaveMakeupData(TorqData::Torque* ptPBData)
{
    ...
    ptPBData->set_bbreakout(g_tGlbCfg.iBreakOut > 0);
...
void CDrillDlg::SaveBreakoutData(TorqData::Torque* ptPBData)
{
    ...
    ptMakeData = &g_tReadData.tData[g_tReadData.nTotal - 1 - g_tReadData.nBreakout];
...
void CDrillDlg::EnableBreakoutfile()
{
    bool bEnable = g_tGlbCfg.iBreakOut > 0;
...
void CDrillDlg::OnBnClickedRadiomakeup()
{
    ...
    ASSERT_ZERO(g_tGlbCfg.iBreakOut);
    if (m_bRunStatus)
    {
