...
{
    COMP_BTRUE_R(m_bRegistered, FALSE);

    ASSERT(pGridCtrl);
    ASSERT(pGridCtrl->IsKindOf(RUNTIME_CLASS(CGridCtrl)));

    ASSERT_NULL_R(pGridCtrl, FALSE);

    COMP_BFALSE_R(pGridCtrl->IsKindOf(RUNTIME_CLASS(CGridCtrl)), FALSE);
...
{
    ASSERT_NULL_R(pWnd, DROPEFFECT_NONE);
