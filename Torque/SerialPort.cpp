    ...
    UINT  writebuffersize) // size to the writebuffer
{
    ...
    else m_ov.hEvent = CreateEvent(NULL, TRUE, FALSE, NULL);
    if (m_hWriteEvent != NULL)
        ResetEvent(m_hWriteEvent);
    else m_hWriteEvent = CreateEvent(NULL, TRUE, FALSE, NULL);
    ...
    if(portnr < 10)
        ...
        sprintf_s(szPort, SPRINTFLEN, "\\\\.\\COM%d", portnr);
    sprintf_s(szBaud, SPRINTFLEN, "baud=%d parity=%c data=%d stop=%d", baud, parity, databits, stopbits);
...
// Write a string to the port
//
void CSerialPort::WriteToPort(char* data, int len)
{
    assert(m_hComm != 0);
    ...
    memset(m_szWriteBuffer, 0, sizeof(m_szWriteBuffer));
    memcpy(m_szWriteBuffer, data, len);
    m_nWriteSize = len;
    LeaveCriticalSection(&m_csCommunicationSync);
    // set event for write
    SetEvent(m_hWriteEvent);
